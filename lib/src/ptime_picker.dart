  ...
  Widget build(BuildContext context) {
    ...
    final Color backgroundColor = timePickerTheme.dayPeriodColor ??
        MaterialStateColor.resolveWith((Set<MaterialState> states) {
          ...
          // visible.
          return states.contains(MaterialState.selected) ? colorScheme.primary.withOpacity(isDark ? 0.24 : 0.12) : Colors.transparent;
        });
    ...
    OutlinedBorder shape = timePickerTheme.dayPeriodShape ?? const RoundedRectangleBorder(borderRadius: _kDefaultBorderRadius);
  ...
  int? _parseHour(String value) {
    ...
    if (MediaQuery.of(context).alwaysUse24HourFormat) {
      ...
      if (newHour > 0 && newHour < 13) {
        if ((_selectedTime!.period == DayPeriod.pm && newHour != 12) || (_selectedTime!.period == DayPeriod.am && newHour == 12)) {
    ...
    final Widget actions = Row(
      children: <Widget>[
        ...
        IconButton(
          ...
          icon: _entryMode == PTimePickerEntryMode.dialOnly || _entryMode == PTimePickerEntryMode.dialOnly
              ? Icon(null)
              : Icon(_entryMode == PTimePickerEntryMode.dial ? Icons.keyboard : Icons.access_time),
